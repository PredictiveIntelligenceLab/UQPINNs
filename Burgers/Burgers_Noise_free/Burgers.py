...
if __name__ == "__main__":
    ...
    # Generate stochastic initial condtion and the exact initial condition
    X_ii = np.linspace(-1., 1., 500)
    Y_ii = f_initial(X_ii)
    Y_i = f_initial(X_i)
    Y_refi = Y_i
    Y_i = Y_i + noise * np.std(Y_i)*np.random.randn(N_i,Y_dim)
    ...
    plt.plot(X_i, Y_i, 'kx', label = "Non-noisy initial condition", alpha = 1.)
